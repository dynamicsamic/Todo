...
class TestTodoAPI:
    ...
    async def test_list_todos_with_limit_offset(self):
        ...
        resp = await self.client.get(f"/todos/?limit={limit}&offset={offset}")
    ...
    async def test_list_todos_with_unique_filters(self):
        ...
        resp = await self.client.get(
            f"/todos/?owner={owners[0]}&owner={owners[1]}"
        )
        assert resp.status_code == HTTPStatus.OK
    ...
    async def test_list_todos_empty_response(self):
        ...
        resp = await self.client.get(f"/todos/?created_at={date}")
    ...
    async def test_get_todo_with_invalid_todo_id(self):
        resp = await self.client.get("/todos/invalid/")
        assert resp.status_code == HTTPStatus.NOT_FOUND

    async def test_get_todo_invalid_prefetch_id(self):
        resp = await self.client.get("/todos/1/?prefetch_tasks=invalid")
        assert resp.status_code == HTTPStatus.BAD_REQUEST

    @pytest.mark.skip("timezone missmatch between api and db")
    ...
    async def test_update_todo_with_empty_payload(self):
        resp = await self.client.patch("/todos/1/", json={})
        assert resp.status_code == HTTPStatus.BAD_REQUEST

    async def test_update_todo_with_invalid_value(self):
        resp = await self.client.patch("/todos/1/", json={"status": "invalid"})
        assert resp.status_code == HTTPStatus.BAD_REQUEST

    async def test_update_todo_with_extra_attr(self):
        resp = await self.client.patch("/todos/1/", json={"extra": "extra"})
        assert resp.status_code == HTTPStatus.BAD_REQUEST

    async def test_update_todo_does_not_exist(self):
        todo_id = TODO_SAMPLE_SIZE+ 100
        resp = await self.client.patch(f"/todos/{todo_id}/", json={"owner": "owner"})
        assert resp.status_code == HTTPStatus.NOT_FOUND
    ...
    async def test_delete_todo_invalid_todo_id(self):
        resp = await self.client.delete("/todos/-1/")
        assert resp.status_code == HTTPStatus.NOT_FOUND
...
class TestTaskAPI:
    ...
    async def test_list_tasks_with_limit_offset(self):
        ...
        resp = await self.client.get(
            f"/todos/1/tasks/?limit={limit}&offset={offset}"
        )
    ...
    async def test_list_tasks_with_unique_filters(self):
        ...
        resp = await self.client.get(
            f"/todos/1/tasks/?brief={briefs[0]}&brief={briefs[1]}"
        )
        assert resp.status_code == HTTPStatus.OK
    ...
    async def test_list_tasks_empty_response(self):
        date = f"{now():%Y-%m-%d %H:%M:%S}"
        resp = await self.client.get(f"/todos/1/tasks/?created_at={date}")
    ...
    async def test_add_task_invalid_brief_value(self):
        ...
        resp = await self.client.post("/todos/2/tasks/", json=payload)
        assert resp.status_code == HTTPStatus.BAD_REQUEST
    ...
    async def test_get_task_with_invalid_task_id(self):
        resp = await self.client.get("/todos/1/tasks/invalid/")
        assert resp.status_code == HTTPStatus.NOT_FOUND
    ...
    async def test_update_task_with_empty_payload(self):
        resp = await self.client.patch("/todos/1/tasks/1/", json={})
        assert resp.status_code == HTTPStatus.BAD_REQUEST

    async def test_update_task_with_invalid_value(self):
        resp = await self.client.patch(
            "/todos/1/tasks/1/", json={"status": "invalid"}
        )
        assert resp.status_code == HTTPStatus.BAD_REQUEST
    ...
    async def test_update_task_does_not_exist(self):
        ...
        resp = await self.client.patch(
            f"/todos/{invalid_id}/tasks/{invalid_id}/", json={"brief": "brief"}
        )
        assert resp.status_code == HTTPStatus.NOT_FOUND

    async def test_delete_task_valid_task_id(self):
        todo_id, task_id = self.get_task()
        url = f"/todos/{todo_id}/tasks/{task_id}/"
    ...
    async def test_delete_task_invalid_task_id(self):
        resp = await self.client.delete("/todos/1/tasks/-1/")
        assert resp.status_code == HTTPStatus.NOT_FOUND
